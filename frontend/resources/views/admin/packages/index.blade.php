...
<main class="py-3 bg-white grow">
                ...
                <tbody>
                    ...
                    @foreach ($data as $package)
                        <tr class="bg-white border-b hover:bg-gray-50">
                            <td class="w-4 p-4">
                                <div class="flex items-center">
                                    <input id="checkbox-{{ $package->id }}" value="{{ $package->id }}"
                                        type="checkbox" name="packageCheckbox"
                                        x-on:change="rowCheckboxChange($el);"
                                        class="w-4 h-4 checkbox-brand-purple-500">
                                    <label for="checkbox-{{ $package->id }}" class="sr-only">checkbox</label>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                {{ $package->name }}
                            ...
                            <td class="flex justify-center py-4 gap-1">
                                ...
                                <x-button @click="confirmDelete([{{ $package->id }}])"
    ...
    <x-flowbite-modal id="confirmDelete" title="Hapus Package">
        <!--  body -->
        <div class="flex flex-col items-center justify-center p-6">
            <i class="fa-regular fa-circle-question text-brand-purple-500 text-9xl"></i>
            <div class="mt-4">
                <span class="font-bold">Apakah anda yakin untuk menghapus package?</span>
            </div>
        </div>
        <!--  footer -->
        <div
            ...
            <x-button type="button" @click="deletePackages(); hide();"
